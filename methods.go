...
func (c *defaultClient) StatusAll(ctx context.Context, filter TrackerStatus, local bool) ([]*GlobalPinInfo, error) {
	...
	filterStr := ""
	if filter != TrackerStatusUndefined { // undefined filter means "all"
...
// empty GlobalPinInfo.
func WaitFor(ctx context.Context, c Client, fp StatusFilterParams) (*GlobalPinInfo, error) {
...
func newStatusFilter() *statusFilter {
	return &statusFilter{
		In:   make(chan *GlobalPinInfo),
		Out:  make(chan *GlobalPinInfo),
...
func statusReached(target TrackerStatus, gblPinInfo *GlobalPinInfo) (bool, error) {
	for _, pinInfo := range gblPinInfo.PeerMap {
		switch pinInfo.Status {
		case target:
			continue
		case TrackerStatusUndefined, TrackerStatusClusterError, TrackerStatusPinError, TrackerStatusUnpinError:
			return false, fmt.Errorf("error has occurred while attempting to reach status: %s", target.String())
		case TrackerStatusRemote:
			if target == TrackerStatusPinned {
...
func makeSerialFile(fpath string, params *AddParams) (files.Node, error) {
	if fpath == "." {
		cwd, err := os.Getwd()
		if err != nil {
			return nil, err
