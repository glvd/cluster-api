...
func (wait *waitService) Pin(ctx context.Context, in *Pin, out *Pin) error {
	wait.l.Lock()
	defer wait.l.Unlock()
...
func (wait *waitService) Status(ctx context.Context, in cid.Cid, out *GlobalPinInfo) error {
	wait.l.Lock()
	defer wait.l.Unlock()
	if time.Now().After(wait.pinStart.Add(5 * time.Second)) { //pinned
		*out = GlobalPinInfo{
			Cid: in,
			PeerMap: map[string]*PinInfo{
